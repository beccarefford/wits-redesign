...
final class Symphony_Theme_Helper {
	...
	function after_switch_theme( $old_theme_name, $old_theme = false ) {
		...
		// Or if we're updating from Symphony
		else if ( ! empty( $old_theme ) && is_a( $old_theme, 'WP_Theme' ) && $old_theme_name === 'Symphony' )
			// Get Symphony theme mods
			if ( $symphony_theme_mods = $this->get_theme_mods( 'symphony' ) )
				// Store theme mods on the child theme
				$this->set_theme_mods( $symphony_theme_mods );
	}
	...
	function after_setup_theme() {
		...
		$conductor_widget_display_support = array_keys( $conductor_support );
						...
						foreach ( $support as $support_type => $support_value ) {
							...
							if ( $support_type !== 'labels' ) {
								switch ( $support_type ) {
									...
									case 'font_size':
										// Font Size
										add_filter( 'symphony_conductor_' . $display . '_' . $support_id . '_' . $support_type, array( $this, 'theme_mod_conductor_filter' ), 10, 2 );

										// Font Size Minimum
										add_filter( 'symphony_conductor_' . $display . '_' . $support_id . '_' . $support_type . '_min', array( $this, 'theme_mod_conductor_filter' ), 10, 2 );

										// Font Size Maximum
										add_filter( 'symphony_conductor_' . $display . '_' . $support_id . '_' . $support_type . '_max', array( $this, 'theme_mod_conductor_filter' ), 10, 2 );
									break;
	 ...
	 */
	function theme_mod_conductor_filter( $value, $default = false ) {
				...
				foreach ( $widget_display_support as $support_id => $support ) {
					...
					if ( $support_id !== 'labels' ) {
						...
						foreach ( $support as $support_type => $support_value ) {
							...
							if ( $support_type !== 'labels' ) {
								// Font Size/Family
								$filters = array( 'symphony_conductor_' . $display . '_' . $support_id . '_' . $support_type );
								...
								switch ( $support_type ) {
									...
									case 'font_size':
										// Font Size Minimum
										$filters[] = 'symphony_conductor_' . $display . '_' . $support_id . '_' . $support_type . '_min';

										// Font Size Maximum
										$filters[] = 'symphony_conductor_' . $display . '_' . $support_id . '_' . $support_type . '_max';
									break;

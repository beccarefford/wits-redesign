<?php
/**
 * Symphony Customizer Conductor (Conductor Customizer functionality)
 */
...
final class Symphony_Customizer_Conductor_Fonts {
	/**
	 * @var string
	 */
	...
	/**
	 * @var Symphony_Customizer_Conductor_Fonts, Instance of the class
	 */
	...
	function __construct() {
		...
		// Filter
		$this->conductor_theme_mods = apply_filters( 'symphony_customizer_conductor_theme_mods', $this->conductor_theme_mods );
		...
		if ( $symphony_theme_helper->current_theme_supports( 'fonts', 'conductor' ) ) {
			...
			if ( ! empty( $conductor_widget_display_support ) ) {
				...
				// 4.0 or greater
					 ...
					 */
					$wp_customize->add_panel( 'symphony_conductor_fonts', array(
						'priority' => 110, // After Conductor Content Layouts
						'title' => __( 'Conductor Typography', 'symphony' )
					) );
				...
				foreach ( $conductor_widget_display_support as $display ) {
					...
					// 4.0 or greater
						...
						// Section
						$wp_customize->add_section( 'symphony_fonts_conductor_' . $display, array(
							'priority' => $section_priority,
							'title' => $widget_display_support['labels']['section'],
							'panel' => 'symphony_conductor_fonts',
							'description' => __( 'Selecting multiple Google Web Fonts may impact the speed/performance of your website.', 'symphony' )
						) );
										...
										switch ( $support_type ) {
											...
											case 'font_size':
												...
												$wp_customize->add_setting(
													'symphony_conductor_' . $display . '_' . $support_id . '_font_size',
													array(
														'default' => apply_filters( 'symphony_conductor_' . $display . '_' . $support_id . '_font_size', 24, 24 ), // Pass the default value as second parameter
												...
												$wp_customize->add_control(
													new Symphony_Customizer_Font_Size_Control(
														...
														'symphony_conductor_' . $display . '_' . $support_id . '_font_size',
														array(
															'label' => sprintf( __( '%1$s Font Size', 'symphony' ), $support['labels']['control'] ),
															'section' => 'symphony_fonts_conductor_' . $display,
															'settings' => 'symphony_conductor_' . $display . '_' . $support_id . '_font_size',
															'priority' => $setting_priority,
															'type' => 'number',
															'input_attrs' => array(
																'min' => apply_filters( 'symphony_conductor_' . $display . '_' . $support_id . '_font_size_min', 18, 18 ), // Pass the default value as second parameter
																'max' => apply_filters( 'symphony_conductor_' . $display . '_' . $support_id . '_font_size_max', 36, 36 ), // Pass the default value as second parameter
																'placeholder' => apply_filters( 'symphony_conductor_' . $display . '_' . $support_id . '_font_size', 24, 24 ), // Pass the default value as second parameter
																'style' => 'width: 70px;'
															),
															'units' => array(
											...
											case 'font_family':
												// Setting
												$wp_customize->add_setting(
													'symphony_conductor_' . $display . '_' . $support_id . '_font_family',
													array(
														'default' => apply_filters( 'symphony_conductor_' . $display . '_' . $support_id . '_font_family', '', '' ), // Pass the default value as second parameter
														'sanitize_callback' => array( $symphony_customizer_fonts, 'sanitize_google_web_font' ),
														'sanitize_js_callback' => array( $symphony_customizer_fonts, 'sanitize_google_web_font' )
													)
												);
												...
												$wp_customize->add_control(
													new Symphony_Customizer_Font_Family_Control(
														...
														'symphony_conductor_' . $display . '_' . $support_id . '_font_family',
														array(
															'label' => sprintf( __( '%1$s Font Family', 'symphony' ), $support['labels']['control'] ),
															'section' => 'symphony_fonts_conductor_' . $display,
															'settings' => 'symphony_conductor_' . $display . '_' . $support_id . '_font_family',
															'priority' => $setting_priority + 5,
															'type' => 'select',
															'choices' => $symphony_customizer_fonts->get_google_fonts_choices()
														)
													)
												);
											break;
	 ...
	 */
	function get_conductor_sidebar_ids() {
	 ...
	 */
	function get_conductor_sidebars() {
		global $wp_registered_sidebars; // Contains the most up-to-date list of registered sidebars at this point
			...
			foreach ( $this->conductor_theme_mods as $theme_mod => $theme_mod_types )
				...
				foreach ( $theme_mod_types as $type ) {
					...
					switch( $type ) {
						// Font Family
						case 'font_family':
							$r[$sidebar_id][$theme_mod][$type] = "'" . get_theme_mod( 'symphony_conductor_' . $sidebar_id . '_' . $theme_mod . '_' . $type ) . "'";
						break;
						// Default
						default:
							$r[$sidebar_id][$theme_mod][$type] = get_theme_mod( 'symphony_conductor_' . $sidebar_id . '_' . $theme_mod . '_' . $type );
	 ...
	 */
	function floatval( $value, $wp_customize_setting ) {
...
/**
 * Create an instance of the Symphony_Customizer_Conductor_Fonts class.
 */
function Symphony_Customizer_Conductor_Fonts() {
	return Symphony_Customizer_Conductor_Fonts::instance();
}

Symphony_Customizer_Conductor_Fonts();
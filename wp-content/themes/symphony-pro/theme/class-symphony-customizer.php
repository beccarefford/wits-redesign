...
class Symphony_Customizer {
	...
	private function includes() {
		...
		if ( class_exists( 'Conductor' ) )
			include_once( 'class-symphony-customizer-conductor-typography.php' ); // Customizer Conductor Typography Class
	...
	function customize_register( $wp_customize ) {
			...
			$wp_customize->add_control(
				new Symphony_Customizer_Checkbox_Control(
					...
					array(
						...
						//'description' => __( 'When "show" is displayed, the footer branding will be displayed on your site and vise-versa.', 'symphony' ),
						'section'  => 'symphony_design_show_hide',
				...
				new WP_Customize_Control(
					...
					array(
						...
						'choices' => array(
							...
							'traditional' => __( 'Default', 'symphony' ),
		...
		if ( $this->version_compare( '4.0' ) ) {
			...
			if ( $primary_color_control = $wp_customize->get_control( 'primary_color' ) ) { // Get Control
				$primary_color_control->section = 'symphony_main_header_navigation'; // Adjust Section
				$primary_color_control->priority = 10; // Adjust Priority
				$primary_color_control->label = __( 'Color', 'symphony' );
			}
		...
		if ( $this->version_compare( '4.0' ) ) {
			...
			if ( $secondary_color_control = $wp_customize->get_control( 'secondary_color' ) ) { // Get Control
				$secondary_color_control->section = 'symphony_secondary_header_navigation'; // Add panel
				$secondary_color_control->priority = 10; // Adjust Priority
				$secondary_color_control->label = __( 'Color', 'symphony' );
			}
		...
		if ( $this->version_compare( '4.0' ) ) {
			...
			if ( $link_color_control = $wp_customize->get_control( 'link_color' ) ) { // Get Control
				$link_color_control->section = 'symphony_content_colors'; // Adjust Section
				$link_color_control->label = __( 'Link', 'symphony' ); // Adjust Label
		...
		if ( $this->version_compare( '4.0' ) ) {
			 ...
			 */
			$wp_customize->add_section( 'symphony_footer_copyright_branding', array(
				'priority' => 20, // After Colors
			...
			$wp_customize->add_setting(
				...
				array(
					'default' => ( ! isset( $sds_theme_options['footer']['copyright'] ) || empty( $sds_theme_options['footer']['copyright'] ) ) ? sds_get_copyright() : $sds_theme_options_defaults['footer']['copyright'],
					'type' => 'option',
					// Data is also sanitized upon update_option() call using the sanitize function in $sds_theme_options_instance
					'sanitize_callback' => array( $this, 'sanitize_footer_copyright_branding' )
				)
			);
			...
			$wp_customize->add_control(
				new WP_Customize_Control(
					...
					array(
						...
						'section' => 'symphony_footer_copyright_branding',
			...
			$wp_customize->add_setting(
				...
				array(
					'default' => ( ! isset( $sds_theme_options['footer']['branding'] ) || empty( $sds_theme_options['footer']['branding'] ) ) ? sds_get_copyright_branding() : $sds_theme_options_defaults['footer']['branding'],
					'type' => 'option',
					// Data is also sanitized upon update_option() call using the sanitize function in $sds_theme_options_instance
					'sanitize_callback' => array( $this, 'sanitize_footer_copyright_branding' )
				)
	 ...
	 */
	function option_sds_theme_options( $value ) {
	...
	function get_customizer_css() {
		...
		// Otherwise return data
			...
			if ( ( $theme_mod_symphony_max_width = $this->get_theme_mod( 'symphony_max_width', 1600 ) ) ) {
				...
				// Conductor (half width on 3 column middle layout)
				if ( class_exists( 'Conductor' ) ) {
					$r .= '.conductor-cols-3-m.fluid-width .content-container div.in {' . "\n";
						$r .= 'max-width: ' . ( $theme_mod_symphony_max_width / 2 ) . 'px;' . "\n";
					$r .= '}' . "\n\n";

					// Media Queries
					$r .= '@media only screen and (max-width: 960px) {' . "\n";
						$r .= '.conductor-cols-3-m.fluid-width .conductor-content {' . "\n";
							$r .= 'float: right;' . "\n";
						$r .= '}' . "\n";
					$r .= '}' . "\n\n";
					...
					$r .= '@media only screen and (max-width: ' . $theme_mod_symphony_max_width. 'px) {' . "\n";
						$r .= '.conductor-cols-3-m.fluid-width .content-container div.in {' . "\n";
							$r .= 'max-width: none;' . "\n";
						$r .= '}' . "\n";
					$r .= '}' . "\n\n";
			...
			if ( ( $theme_mod_content_color = $this->get_theme_mod( 'content_color', $this->theme_mod_content_color() ) ) ) {
				...
				// Conductor
				if ( class_exists( 'Conductor' ) ) {
					$r .= '/* Conductor Widget - Content Color */' . "\n";
					$r .= '.conductor-widget {' . "\n";
						$r .= 'color: ' . $theme_mod_content_color . ';' . "\n";
					$r .= '}' . "\n\n";
				}
			...
			if ( ( $theme_mod_link_color = $this->get_theme_mod( 'link_color', $this->theme_mod_link_color() ) ) ) {
				...
				// Conductor
				if ( class_exists( 'Conductor' ) ) {
					$r .= '/* Conductor Widget Link Color */' . "\n";
					$r .= '.conductor-widget a {' . "\n";
						$r .= 'color: ' . $theme_mod_link_color . ';' . "\n";
					$r .= '}' . "\n\n";
				}
			}
			...
			if ( ( $theme_mod_symphony_post_title_color = $this->get_theme_mod( 'symphony_post_title_color', $this->theme_mod_symphony_post_title_color() ) ) ) {
				...
				// Conductor
				if ( class_exists( 'Conductor' ) ) {
					$r .= '/* Conductor Widget Post Title Color */' . "\n";
					$r .= '.conductor-widget .post-title a, .conductor-widget .post-title a:hover {' . "\n";
						$r .= 'color: ' . $theme_mod_symphony_post_title_color . ';' . "\n";
					$r .= '}' . "\n\n";
				}
			}
			...
			if ( ( $theme_mod_symphony_button_text_color = $this->get_theme_mod( 'symphony_button_text_color', $this->theme_mod_symphony_button_text_color() ) ) ) {
				...
				// Conductor
				if ( class_exists( 'Conductor' ) ) {
					$r .= '/* Conductor Widget Button Text Color */' . "\n";
					$r .= '.conductor-widget .post-content .more-link {' . "\n";
						$r .= 'color: ' . $theme_mod_symphony_button_text_color . ';' . "\n";
						$r .= 'border-color: ' . $theme_mod_symphony_button_text_color . ';' . "\n";
						if ( ! $this->get_theme_mod( 'symphony_button_background_color', $this->theme_mod_symphony_button_background_color() ) )
							$r .= 'background-color: ' . $this->theme_mod_symphony_button_background_color() . ';' . "\n";
				...
				// Conductor
				if ( class_exists( 'Conductor' ) ) {
					$r .= '/* Conductor Widget Button Text Color Hover */' . "\n";
					$r .= '.conductor-widget .post-content .more-link:hover {' . "\n";
						if ( ! $this->get_theme_mod( 'symphony_button_background_color', $this->theme_mod_symphony_button_background_color() ) ) {
							$r .= 'color: ' . $this->theme_mod_symphony_button_background_color() . ';' . "\n";
							$r .= 'border-color: ' . $this->theme_mod_symphony_button_background_color() . ';' . "\n";
						}
			...
			if ( ( $theme_mod_symphony_button_background_color = $this->get_theme_mod( 'symphony_button_background_color', $this->theme_mod_symphony_button_background_color() ) ) ) {
				...
				// Conductor
				if ( class_exists( 'Conductor' ) ) {
					$r .= '/* Conductor Widget Button Background Color */' . "\n";
					$r .= '.conductor-widget .post-content .more-link {' . "\n";
						$r .= 'background-color: ' . $theme_mod_symphony_button_background_color . ';' . "\n";
					$r .= '}' . "\n\n";
				}
				...
				// Conductor
				if ( class_exists( 'Conductor' ) ) {
					$r .= '/* Conductor Widget Button Background Color Hover */' . "\n";
					$r .= '.conductor-widget .post-content .more-link:hover{' . "\n";
						$r .= 'color: ' . $theme_mod_symphony_button_background_color . ';' . "\n";
					$r .= '}' . "\n\n";
				}
	...
	/**
	 * This function sanitizes footer branding and copyright data.
	 */
	function sanitize_footer_copyright_branding( $value ) {

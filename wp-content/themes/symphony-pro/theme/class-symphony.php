...
class Symphony {
	...
	function __construct() {
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'admin_enqueue_scripts' ) ); // Admin scripts/styles
		...
		add_action( 'conductor_widget_post_content_before', array( $this, 'conductor_widget_post_content_before' ), 20, 2 ); // Output a post content wrapper on table displays
		add_action( 'conductor_widget_post_content_after', array( $this, 'conductor_widget_post_content_after' ), 5, 2 ); // Close post content wrapper on table displays
		add_filter( 'conductor_widget_defaults', array( $this, 'conductor_widget_defaults' ) ); // Adjust Conductor widget defaults
		add_filter( 'conductor_widget_displays', array( $this, 'conductor_widget_displays' ), 10, 3 ); // Add Portfolio Conductor widget display
		add_action( 'conductor_widget_settings_display_preview_portfolio', array( $this, 'conductor_widget_settings_display_preview_portfolio' ), 10, 2 ); // Portfolio Conductor widget display preview
		add_filter( 'conductor_widget_available_image_sizes', array( $this, 'conductor_widget_available_image_sizes' ) ); // Add "full" image size to available image sizes
		...
		add_action( 'conductor_widget_pagination_before', array( $this, 'conductor_widget_pagination_before' ) );
		add_action( 'conductor_widget_pagination_after', array( $this, 'conductor_widget_pagination_after' ) );
		add_action( 'conductor_widget_after', array( $this, 'conductor_widget_after' ) ); // Output a clearing element after Grid Display layouts
	...
	function after_setup_theme() {
		...
		add_image_size( 'symphony-conductor-small', 375, 9999, false ); // Conductor Small Widgets
		add_image_size( 'symphony-conductor-small-cropped', 375, 250, true ); // Conductor Small Widgets (cropped)
		add_image_size( 'symphony-conductor-medium', 760, 9999, false ); // Conductor Medium Widgets
		add_image_size( 'symphony-conductor-medium-cropped', 760, 500, true ); // Conductor Medium Widgets (cropped)
		add_image_size( 'symphony-conductor-large', 1600, 9999, false ); // Conductor Large Widgets
		add_image_size( 'symphony-conductor-large-cropped', 1600, 1600, false ); // Conductor Large Widgets (cropped)
			...
			'fonts' => array(
				...
				'conductor' => array(
					...
					'large' => array(
						...
						'labels' => array(
							// Customizer Section
							'section' => __( 'Conductor: Large Widget Display', 'symphony' )
						),
						// Title (Post Title)
						'title' => array(
							// Labels
						...
						'author_byline' => array(
							...
							'labels' => array(
								// Customizer Control (prefix)
								'control' => __( 'Author Byline', 'symphony' )
							),
							// Font Size
							'font_size' => array(
								'default' => 15,
					...
					'medium' => array(
						...
						'labels' => array(
							// Customizer Section
							'section' => __( 'Conductor: Medium Widget Display', 'symphony' )
						),
						// Title (Post Title)
						'title' => array(
							// Labels
					...
					'small' => array(
						...
						'labels' => array(
							// Customizer Section
							'section' => __( 'Conductor: Small Widget Display', 'symphony' )
						),
						// Title (Post Title)
						'title' => array(
							// Labels
					...
					'list' => array(
						...
						'labels' => array(
							// Customizer Section
							'section' => __( 'Conductor: List Widget Display', 'symphony' )
						),
						// Title (Post Title)
						'title' => array(
							// Labels
					...
					'table' => array(
						...
						'labels' => array(
							// Customizer Section
							'section' => __( 'Conductor: Table Widget Display', 'symphony' )
						),
						// Title (Post Title)
						'title' => array(
							// Labels
					...
					'grid' => array(
						...
						'labels' => array(
							// Customizer Section
							'section' => __( 'Conductor: Grid Widget Display', 'symphony' )
						),
						// Title (Post Title)
						'title' => array(
							// Labels
					...
					'portfolio' => array(
						...
						'labels' => array(
							// Customizer Section
							'section' => __( 'Conductor: Portfolio Widget Display', 'symphony' )
						),
						// Title (Post Title)
						'title' => array(
							// Labels
	...
	function after_switch_theme( $old_theme_name, $old_theme = false ) {
		...
		if ( ! is_array( $old_sidebars_widgets ) ) {
			...
			$this->wp_sidebars_changed( ( ! empty( $old_theme ) && is_a( $old_theme, 'WP_Theme' ) && $old_theme_name === 'Symphony' ) ? true : false );
		}
	...
	function admin_notices() {
    	printf( __( '<div class="updated"><p>Welcome to Symphony Pro! Get started by visiting the <a href="%1$s">Customizer</a>!</p></div>', 'symphony' ), esc_url( wp_customize_url() ) );
	}
	...
	/**
	 * This function enqueues the necessary scripts and styles associated with the admin.
	 */
	function admin_enqueue_scripts( $hook ) {
		// Only on Widgets Admin Page
		if ( $hook === 'widgets.php' )
			wp_enqueue_style( 'symphony-conductor-admin', get_template_directory_uri() . '/css/symphony-conductor-admin.css', array( 'conductor-widget-admin' ) );
	}
	...
	function sds_theme_options_customizer_section() {
		printf( __( 'Welcome to Symphony. Symphony offers many more options in the <a href="%1$s">Customizer</a> and also works great with our <a href="%2$s" target="_blank">Conductor</a> plugin!','symphony' ), wp_customize_url(), esc_url( 'http://conductorplugin.com/?utm_source=symphony&utm_medium=link&utm_content=conductor-symphony&utm_campaign=conductor' ) );
	}
	...
	function admin_init() {
		...
		$this->_updater = new EDD_SL_Theme_Updater( array(
			...
			'author' 	=> __( 'Slocum Studio', 'symphony' )
		) );
	 ...
	 */
	function conductor_content_wrapper_element_before( $wrapper ) {
		$wrapper = '<div class="content-container conductor-container container conductor-cf">';

		// Fluid Width
		if ( ! symphony_is_fixed_width() )
	 ...
	 */
	function conductor_content_wrapper_element_after( $wrapper ) {
		$wrapper = '';

		// Fluid Width
		if ( ! symphony_is_fixed_width() )
			$wrapper .= '<div class="clear"></div></div>';

		$wrapper .= '<div class="conductor-cf conductor-clear"></div></div>';

		return $wrapper;
	}

	/**
	 * This function outputs an opening content wrapper before post content on Conductor
	 * Table Widget Displays.
	 */
	function conductor_widget_post_content_before( $post, $instance ) {
		// Table display
	...
	/**
	 * This function outputs a closing content wrapper before post content on Conductor
	 * Table Widget Displays.
	 */
	function conductor_widget_post_content_after( $post, $instance ) {
		// Table display
		if ( ! empty( $instance ) && isset( $instance['widget_size'] ) && $instance['widget_size'] === 'table' ) :
		?>
			</div>
	...
	function conductor_widget_defaults( $defaults ) {
		...
		foreach ( $defaults['output'] as &$output )
			...
			if ( $output['id'] === 'read_more' )
				$output['label'] = symphony_more_link_label();

		// Read More output features
		$defaults['output_features']['read_more']['edit_label']['default'] = _x( 'Continue Reading', '"read more" label for Conductor widgets', 'symphony' );

		return $defaults;
	}

	/**
	 * This function outputs a Portfolio display option to Conductor widgets.
	 */
	function conductor_widget_displays( $conductor_widget_displays, $instance, $conductor_widget ) {
		// Portfolio
		if ( ! array_key_exists( 'portfolio', $conductor_widget_displays ) )
			$conductor_widget_displays['portfolio'] = __( 'Portfolio', 'symphony' );

		return $conductor_widget_displays;
	}

	/**
	 * This function outputs the preview for the Portfolio widget display on Conductor widgets
	 */
	function conductor_widget_settings_display_preview_portfolio( $instance, $conductor_widget ){
	?>
		<div class="posts posts-portfolio">
			<div class="post post-portfolio">
				<div class="portfolio dashicons dashicons-images-alt2"></div>
			</div>
		</div>
	<?php
	}
	...
	/**
	 * This function adds a "full" featured image size to Conductor widgets.
	 */
	function conductor_widget_available_image_sizes( $avail_image_sizes ) {
		// Add the "full" size if it doesn't already exist;
		if ( ! array_key_exists( 'full', $avail_image_sizes ) ) {
			$avail_image_sizes['full']['label'] = _x( 'Full', 'full featured image size label for Conductor', 'symphony' );
			$avail_image_sizes['full']['width'] = 9999;
			$avail_image_sizes['full']['height'] = 9999;
		}
	 ...
	 */
	function conductor_widget_featured_image_size( $size, $instance ) {
		...
		if ( isset( $instance['widget_size'] ) ) {
			// Small
			if ( $instance['widget_size'] === 'small' )
				$size = 'symphony-conductor-small';

			// Medium
			if ( $instance['widget_size'] === 'medium' )
				$size = 'symphony-conductor-medium';

			// Large
			if ( $instance['widget_size'] === 'large' )
				$size = 'symphony-conductor-large';

			// Portfolio
			if ( $instance['widget_size'] === 'portfolio' )
				$size = 'symphony-600x400';
		}
	...
	/**
	 * This function outputs a clearing element before pagination on Conductor widgets.
	 */
	...
	/**
	 * This function outputs a clearing element after pagination on Conductor widgets.
	 */
	function conductor_widget_pagination_after() {
	?>
		</div>
	<?php
	}
	...
	/**
	 * This function outputs a clearing element after Grid Display Conductor widgets.
	 */
	function conductor_widget_after( $instance ) {
		// Display Pack Add-On exists
	 ...
	 */
	function wp_sidebars_changed( $symphony_update = false, $ignore_sidebars = array( 'top-sidebar', 'primary-nav-sidebar', 'secondary-header-sidebar' ) ) {
		...
		// Check to see if updating from Symphony to Symphony Pro
		if ( $symphony_update ) {
			// Check ignored sidebars here
			foreach ( $ignore_sidebars as $id ) {
				// if the sidebar exists, copy it to the new set of widgets
				if ( isset( $wp_registered_sidebars[$id] ) ) {
					$_sidebars_widgets[$id] = $sidebars_widgets[$id];
					unset( $sidebars_widgets[$id] ); // Unset sidebar
				}
			}
		}
